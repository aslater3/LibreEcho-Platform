#!/usr/bin/env python3
"""Ownership contract: phone attenuation is media-only; audiod owns PCM master."""
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="radar-bridge-volume-") as directory:
        ...
        for suffix in ("media.volume", "airplay.volume", "airplay.active",
        ...
        subprocess.run([str(binary), "--stop"], check=True, timeout=10)
        if legacy.exists() or (root / "airplay.volume").exists():
            raise SystemExit("disconnect left an AirPlay volume override")
    ...
    print("AirPlay media gain / shared master ownership contract: PASS")
