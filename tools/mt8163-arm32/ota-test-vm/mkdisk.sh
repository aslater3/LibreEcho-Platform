...
# Always release resources, including malformed input and failed mount paths.
LOOP=; MNT=; PROFILE_TMP=
cleanup() {
  rc=$?
  trap - EXIT
  ...
  if [ -n "$MNT" ]; then rmdir "$MNT" || exit 1; fi
  if [ -n "$PROFILE_TMP" ]; then rm -rf "$PROFILE_TMP"; fi
  exit "$rc"
}
trap cleanup EXIT
trap 'exit 130' INT
...
if [ -n "$PROFILE" ]; then
  ...
  python3 /work/parse_profile.py "$PROFILE" "$PROFILE_TMP/config" "$PROFILE_TMP/state"
  { read -r SLOT; read -r ROLLBACK; } < "$PROFILE_TMP/state"
fi
IMG=/work/emmc.img
...
if [ -n "$PROFILE$SCENARIO" ]; then
  ...
  if [ -n "$PROFILE" ]; then
    ...
    cp "$PROFILE_TMP/config" "$MNT/libreecho/config/web-config.json"
  ...
  sync; umount "$MNT"; losetup -d "$LOOP"; LOOP=; rmdir "$MNT"; MNT=
fi
