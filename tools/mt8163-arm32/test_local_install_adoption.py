...
class StagedLocalInstallAdoptionTests(unittest.TestCase):
    ...
    def run_adopt(self, *, image_profile: str = "ota", boot_hash: str | None = None,
                  preflight_rc: int = 0, stub_rc: int = 0) -> subprocess.CompletedProcess[str]:
        ...
        function = extract_function(INIT.read_text(), "adopt_staged_local_install")
    ...
    def test_no_cmdline_slot_is_refused(self) -> None:
        self.cmdline.write_text("console=ttyS0\n")
        result = self.run_adopt()
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(self.invocations(), [])
