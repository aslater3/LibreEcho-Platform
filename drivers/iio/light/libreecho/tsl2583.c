...
static const char *tsl2583x_get_name(struct tsl258x_chip *chip);
...
taos_i2c_read(struct i2c_client *client, u8 reg, u8 *val, unsigned int len)
{
	...
	for (i = 0; i < len; i++) {
		...
		if (ret < 0) {
			dev_err(&client->dev,
				"taos_i2c_read failed to read register %x\n",
				reg);
			return ret;
		}
		*val = ret;
...
#ifdef CONFIG_ALS_TSL2584_AUTOZERO
	cancel_delayed_work(&chip->autozero_work);
#endif
	ret = i2c_smbus_write_byte_data(chip->client,
					TSL258X_CMD_REG | TSL258X_CNTRL,
					0x00);
...
static int parse_alscal_idme(int *coeff)
{
	...
	strsep(&cursor, " ");
	if (!cursor ||
	    sscanf(cursor, "ams_400_0=%d,%d", &dummy, coeff) != 2) {
...
static DEFINE_SIMPLE_DEV_PM_OPS(taos_pm_ops, taos_suspend, taos_resume);
