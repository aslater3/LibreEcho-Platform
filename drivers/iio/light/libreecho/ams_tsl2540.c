...
static int __maybe_unused tsl2540_pltf_power_off(struct tsl2540_chip *chip)
{
	int rc = 0;
...
static int __maybe_unused tsl2540_add_sysfs_interfaces(struct device *dev,
...
static void __maybe_unused tsl2540_remove_sysfs_interfaces(struct device *dev,
	struct device_attribute *a, int size)
{
	int i;
...
static int tsl2540_get_id(struct tsl2540_chip *chip, u8 *id, u8 *rev, u8 *auxid)
{
	...
	ret = ams_i2c_read(chip->client, TSL2540_REG_AUXID, auxid);
	if (ret < 0)
		return ret;
	ret = ams_i2c_read(chip->client, TSL2540_REG_REVID, rev);
	if (ret < 0)
		return ret;
	ret = ams_i2c_read(chip->client, TSL2540_REG_ID, id);
	if (ret < 0)
		return ret;
...
static int __maybe_unused tsl2540_power_on(struct tsl2540_chip *chip)
{
	int rc;
...
static struct i2c_driver tsl2540_driver = {
	.driver = {
		.name = "tsl2540",
		.pm = TSL2540_PM_OPS,
		.of_match_table = of_match_ptr(tsl2540_i2c_dt_ids),
	},
