...
#ifdef CONFIG_MTK_MT8163_BLUEZ_HCI
...
static void mtk_bt_hci_receive(const UINT8 *data, INT32 size);

static int mtk_bt_hci_open(struct hci_dev *hdev)
{
	int result;

	pr_info("ECHO_BT_HCI_OPEN: begin\n");

	result = mtk_wcn_wmt_func_on(WMTDRV_TYPE_BT);
	if (result != MTK_WCN_BOOL_TRUE) {
		pr_err("ECHO_BT_HCI_OPEN: FUNC_ON failed ret=%d\n", result);
		return -ENODEV;
	}
	pr_info("ECHO_BT_HCI_OPEN: FUNC_ON ok ready=%d\n",
	...
	pr_info("ECHO_BT_HCI_OPEN: RX register ret=%d\n", result);
	if (result) {
	...
	pr_info("ECHO_BT_HCI_OPEN: complete\n");
	return 0;
}
...
static void mtk_bt_hci_work(struct work_struct *work)
{
	...
	while ((skb = skb_dequeue(&hci->txq))) {
		int result;
		UINT8 pkt_type = bt_cb(skb)->pkt_type;

		skb_push(skb, 1);
		skb->data[0] = pkt_type;
		pr_info("ECHO_BT_HCI_TX: type=0x%02x len=%u\n", pkt_type, skb->len);
		result = mtk_wcn_stp_send_data(skb->data, skb->len,
					       BT_TASK_INDX);
		pr_info("ECHO_BT_HCI_TX: result=%d\n", result);
		if (result <= 0) {
...
static void mtk_bt_hci_receive(const UINT8 *data, INT32 size)
{
	...
	if (!mtk_hci.hdev || size <= 0)
		return;
	pr_info("ECHO_BT_HCI_RX: size=%d type=0x%02x\n", size, data[0]);

	/* The STP transport delivers a complete HCI frame whose first byte is
	 * the packet type (command/ACL/SCO/event).  Modern kernels removed
	 * hci_recv_stream_fragment(); hand a full skb to hci_recv_frame(),
	 * which reads the packet type from bt_cb(skb)->pkt_type. */
	if (size < 1)
		return;
	skb = bt_skb_alloc(size - 1, GFP_ATOMIC);
	if (!skb) {
		BT_ERR_FUNC("hci receive alloc failed: %d\n", size);
		return;
	}
	bt_cb(skb)->pkt_type = data[0];
	memcpy(skb_put(skb, size - 1), data + 1, size - 1);
	skb->dev = (void *)mtk_hci.hdev;
...
static int mtk_bt_hci_init(void)
{
	...
	set_bit(HCI_QUIRK_BROKEN_LOCAL_EXT_FEATURES_PAGE_2, &mtk_hci.hdev->quirks);
	...
	if (result) {
		...
		hci_dev_clear_flag(mtk_hci.hdev, HCI_AUTO_OFF);
	}
	return result;
}
