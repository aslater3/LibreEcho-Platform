...
static void osal_timer_callback(struct timer_list *timer)
{
	P_OSAL_TIMER pTimer = from_timer(pTimer, timer, timer);

	pTimer->timeoutHandler(pTimer->timeroutHandlerData);
}

INT32 osal_timer_create(P_OSAL_TIMER pTimer)
{
	timer_setup(&pTimer->timer, osal_timer_callback, 0);
	return 0;
}
...
INT32 osal_wake_lock_init(P_OSAL_WAKE_LOCK pLock)
{
	...
	#ifdef MT8163_CONN_PM_WAKEUP_SOURCE
...
INT32 osal_wake_lock_count(P_OSAL_WAKE_LOCK pLock)
{
	...
	#ifdef MT8163_CONN_PM_WAKEUP_SOURCE
	count = pLock->wake_lock && pLock->wake_lock->active;
	#else
