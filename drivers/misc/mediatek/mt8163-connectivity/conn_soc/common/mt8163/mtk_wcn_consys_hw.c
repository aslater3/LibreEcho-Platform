...
INT32 mtk_wcn_consys_power_on(VOID)
{
	INT32 iRet = -1;
...
#else
	iRet = pm_runtime_resume_and_get(&my_pdev->dev);
	if (iRet < 0) {
		WMT_PLAT_ERR_FUNC("pm_runtime_resume_and_get() fail(%d)\n", iRet);
		return iRet;
	}
	WMT_PLAT_INFO_FUNC("pm_runtime_resume_and_get() CONSYS ok\n");
...
/*step2.MTCMOS ctrl*/
		...
		iRet = mtk_wcn_consys_power_on();
		if (iRet) {
			WMT_PLAT_ERR_FUNC("ECHO_CONSYS: power-on failed(%d)\n",
					  iRet);
			return iRet;
		}
#else
...
INT32 mtk_wcn_consys_hw_wifi_paldo_ctrl(UINT32 enable)
{
	...
	if (enable) {
		...
		if (IS_ERR_OR_NULL(reg_VCN33_WIFI)) {
			...
			ret = regulator_set_voltage(reg_VCN33_WIFI, 3300000, 3300000);
			if (!ret)
