...
kalDevPortRead(IN P_GLUE_INFO_T GlueInfo, IN UINT_16 Port, IN UINT_32 Size, OUT PUINT_8 Buf, IN UINT_32 MaxBufSize)
{
	...
	if ((HifInfo->fgDmaEnable == TRUE) && (HifInfo->DmaOps != NULL)
		...
		/*
		 * The firmware-download status path passes a vmalloc-backed
		 * buffer here.  Modern DMA API validation rejects streaming
		 * maps of vmalloc memory (dma_map_single() warns and returns
		 * DMA_MAPPING_ERROR, then the cache-sync Oopses).  Use a
		 * kmalloc bounce buffer for the DMA and copy the result back
		 * into the caller's buffer afterwards.  RxBounce/RxBounceDma
		 * are function-scope so the existing teardown below can unmap
		 * and copy back.
		 */
		RxBounce = kmalloc(Size, GFP_KERNEL);
		if (RxBounce == NULL) {
			DBGLOG(RX, ERROR, "kalDevPortRead: rx bounce alloc failed (%u)\n", Size);
			return FALSE;
		}
		DmaConf.Dst = dma_map_single(HifInfo->Dev, RxBounce, Size, DMA_FROM_DEVICE);
		if (dma_mapping_error(HifInfo->Dev, DmaConf.Dst)) {
			DBGLOG(RX, ERROR, "kalDevPortRead: rx bounce dma map failed (%u)\n", Size);
			kfree(RxBounce);
			RxBounce = NULL;
			return FALSE;
		}
...
/* UCHAR AeeBuffer[100]; */
				...
				HifRegDump(GlueInfo->prAdapter);
#endif
				/* Release the RX bounce buffer before bailing out so the
				 * streaming DMA mapping and kmalloc do not leak. */
				if (RxBounce != NULL) {
...
static int HifAhbProbe(VOID)
{
	...
	/* power on WiFi TX PA 3.3V and HIF GDMA clock */
	{
		...
		i4RailRet = mtk_wcn_consys_hw_wifi_paldo_ctrl(1);
		if (i4RailRet) {
			pr_err("ECHO_WIFI_RAIL: WLAN probe blocked ret=%d\n", i4RailRet);
			return i4RailRet;
		}
