...
{
	UINT_32 cpupcr, wc, wasr, d2h0, d2h1;
	...
	pr_err("ECHO_FW_EXEC: %s CPUPCR=0x%08x WCIR=0x%08x POR=%u READY=%u "
	       "WASR=0x%08x D2HRM0=0x%08x D2HRM1=0x%08x cpu=%u jiffies=%lu\n",
	       pszStage, cpupcr, wc, !!(wc & WCIR_POR_INDICATOR),
	       !!(wc & WCIR_WLAN_READY), wasr, d2h0, d2h1,
		 ...
		 IN P_REG_INFO_T prRegInfo, IN PVOID pvFwImageMapFile, IN UINT_32 u4FwImageFileLength)
{
	...
	do {
		...
		echoWlanExecutionSnapshot(prAdapter, "before-start");
...
WLAN_STATUS wlanQueryNicCapability(IN P_ADAPTER_T prAdapter)
{
	...
	pr_err("ECHO_WIFI_CAP: feature_set=0x%08x hw_5g_disabled=%u eeprom=%u efuse=%u\n",
...
WLAN_STATUS wlanLoadManufactureData(IN P_ADAPTER_T prAdapter, IN P_REG_INFO_T prRegInfo)
{
	...
	if (prAdapter->rVerInfo.u2Part1CfgOwnVersion == 0 &&
	    prAdapter->rVerInfo.u2Part1CfgPeerVersion == 0 &&
	    prAdapter->rVerInfo.u2Part2CfgOwnVersion == 0 &&
	    prAdapter->rVerInfo.u2Part2CfgPeerVersion == 0) {
		/* This development image has no board-specific Wi-Fi NVRAM file.
		 * Keep the compiled defaults (including 5 GHz support) instead of
		 * returning before wlanLoadManufactureData() applies them. */
		pr_info("ECHO_WIFI_NVRAM: no versioned config; using compiled defaults\n");
	} else if (CFG_DRV_OWN_VERSION < prAdapter->rVerInfo.u2Part1CfgPeerVersion
	...
	pr_info("ECHO_WIFI_BAND: enable=%d nvram_enable=%u support5g=%u hw_disabled=%u\n",
