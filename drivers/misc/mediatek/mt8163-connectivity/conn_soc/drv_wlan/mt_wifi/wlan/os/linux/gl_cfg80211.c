		     ...
		     const u8 *mac_addr, struct key_params *params)
{
	...
	rKey.u4Length = offsetof(PARAM_KEY_T, aucKeyMaterial) +
			rKey.u4KeyLength;
 ...
 */
/*----------------------------------------------------------------------------*/
int mtk_cfg80211_set_default_mgmt_key(struct wiphy *wiphy,
...
int mtk_cfg80211_connect(struct wiphy *wiphy, struct net_device *ndev, struct cfg80211_connect_params *sme)
{
	...
	if (sme->channel) {
		...
		rNewSsid.rChannelInfo.ucChannelNum =
			nicFreq2ChannelNum(sme->channel->center_freq * 1000);
		rNewSsid.rChannelInfo.eBand =
			sme->channel->band == IEEE80211_BAND_5GHZ ?
			BAND_5G : BAND_2G4;
		rNewSsid.eChnlSco = CHNL_EXT_SCN;
	} else {
			      ...
			      IN struct net_device *ndev, IN struct cfg80211_sched_scan_request *request)
{
	...
	prSchedScanRequest->u2ScanInterval =
		(UINT_16)(request->scan_plans[0].interval);
