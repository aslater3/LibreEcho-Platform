...
kalIndicateStatusAndComplete(IN P_GLUE_INFO_T prGlueInfo, IN WLAN_STATUS eStatus, IN PVOID pvBuf, IN UINT_32 u4BufLen)
{
	...
	case WLAN_STATUS_SCAN_COMPLETE:
		...
		if (prScanRequest != NULL)
			cfg80211_scan_done(prScanRequest,
				&(struct cfg80211_scan_info) { .aborted = false });
		break;
...
static void kal_os_timer_callback(struct timer_list *timer)
{
	P_GLUE_INFO_T prGlueInfo = from_timer(prGlueInfo, timer, tickfn);

	prGlueInfo->tickfn_handler((unsigned long)prGlueInfo);
}
...
VOID kalOsTimerInitialize(IN P_GLUE_INFO_T prGlueInfo, IN PVOID prTimerHandler)
{
	...
	prGlueInfo->tickfn_handler =
		(void (*)(unsigned long data))prTimerHandler;
	timer_setup(&prGlueInfo->tickfn, kal_os_timer_callback, 0);
}
