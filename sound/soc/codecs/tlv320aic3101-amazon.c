...
static void aic31xx_apply_tuning(struct snd_soc_component *component)
{
	...
	for (i = 0; i < config_size; i++) {
		...
		snd_soc_component_write(component, offset, val);
	}

	/* Set the signal processing block (PRB) modes */
	for (i = 0; i < NUM_ADC3101; i++)
		snd_soc_component_write(component, ADC_PRB_SELECT(i), 0x2);
}
 ...
 *----------------------------------------------------------------------------
 */
static int adc31xx_change_page(unsigned int device, struct snd_soc_component *component,
			       u8 new_page)
{
	struct aic31xx_priv *adc31xx = snd_soc_component_get_drvdata(component);
	...
	if (i2c_master_send(adc31xx->adc_control_data[device], data, 2) != 2) {
		dev_err(component->dev, "adc31xx_change_page %d: I2C Wrte Error\n",
			device);
		return -1;
	}
 ...
 *----------------------------------------------------------------------------
 */
static inline void aic31xx_write_reg_cache(struct snd_soc_component *component, u8 device,
					   u8 page, u16 reg, u8 value)
{
	struct aic31xx_priv *aic31xx =
		snd_soc_component_get_drvdata(component);
	u8 *cache = aic31xx->reg_cache;
	int offset;
	...
	dev_dbg(component->dev, "(%d) wrote %#x to %#x (%d) cache offset %d\n",
 ...
 *----------------------------------------------------------------------------
 */
static unsigned int aic31xx_read_reg_cache(struct snd_soc_component *component,
					   unsigned int reg)
{
	struct aic31xx_priv *aic31xx =
		snd_soc_component_get_drvdata(component);
	...
	if (page > 1) {
		dev_warn(component->dev, "unable to read register: page:%u, reg:%u\n",
			 page, reg_no);
		return -EINVAL;
	}
	...
	dev_dbg(component->dev, "(%d) read %#x from %#x (%d) cache offset %d\n",
 ...
 *----------------------------------------------------------------------------
 */
static int aic31xx_write(struct snd_soc_component *component, unsigned int reg,
			 unsigned int value)
{
	int ret = 0;
	u8 data[2];
	...
	dev_dbg(component->dev, "AIC Write dev %#x page %d reg %#x val %#x\n",
	...
	if (aic31xx->adc_page_no[device] != page)
		adc31xx_change_page(device, component, page);
	...
	if (i2c_master_send(i2cdev, data, 2) != 2) {
		dev_err(component->dev, "Error in i2c write in i2c dev: %x \n",
			device);

		ret = -EIO;
	} else if ((page < 2) && !((page == 0) && (reg == 1))) {
		/* Shadow the first two pages only */
		aic31xx_write_reg_cache(component, device, page, reg_no, value);
	}
...
static int aic31xx_add_widgets(struct snd_soc_component *component)
{
	...
	dev_dbg(component->dev, "###aic31xx_add_widgets\n");
...
static void tlv320aic3101_sw_reset(struct snd_soc_component *component)
{
	int i;

	/* # Software Reset */
	for (i = 0; i < NUM_ADC3101; i++)
		snd_soc_component_write(component, ADC_RESET(i), 0x1);
...

{
	...
	dev_dbg(component->dev, "tx_mask=%u rx_mask=%u slots=%d slot_width=%d\n",
...
static int aic31xx_dai_apply_tdm_slot(struct snd_soc_component *component)
{
	...
	if (ret) {
		dev_err(component->dev, "Can't get mic position, so use the default position.\n", ret);
		mic_pos = 0;
	}
#endif
	...
	if (mic_pos == 0) {
		if (aic31xx->tdm_rx_mask == 0x40) {
			dev_info(component->dev, "linein enabled slot_width=%d\n",
		...
		for (i = 0; i < NUM_ADC3101; i++) {
			...
			snd_soc_component_write(component, ADC_I2S_TDM_CTRL(i),
				(enable_channels << ADC_TDM_CHANNEL_EN_SHIFT) | EARLY_3STATE_ENABLED | swap_channels);
			snd_soc_component_write(component, ADC_CH_OFFSET_1(i),
		...
		for (i = 0; i < NUM_ADC3101; i++) {
			 ...
			 */
			snd_soc_component_write(component, ADC_CH_OFFSET_1(i), mic_pos_offset[2*i]);
			snd_soc_component_write(component, ADC_CH_OFFSET_2(i), mic_pos_offset[2*i + 1]);
			snd_soc_component_update_bits(component, ADC_I2S_TDM_CTRL(i),
...
static void tlv320aic3101_save_settings(struct snd_soc_component *component)
{
	int i;

	for (i = 0; i < NUM_ADC3101; i++) {
		/* Save left and right ADC PGA input selection */
		pga1_input_sel_l[i] = snd_soc_component_read(component, ADC_LEFT_PGA_SEL_1(i));
		pga1_input_sel_r[i] = snd_soc_component_read(component, ADC_RIGHT_PGA_SEL_1(i));
		pga2_input_sel_l[i] = snd_soc_component_read(component, ADC_LEFT_PGA_SEL_2(i));
		pga2_input_sel_r[i] = snd_soc_component_read(component, ADC_RIGHT_PGA_SEL_2(i));

		/* Save PGA gain */
		mic_pga_l[i] = snd_soc_component_read(component, ADC_LEFT_APGA_CTRL(i));
		mic_pga_r[i] = snd_soc_component_read(component, ADC_RIGHT_APGA_CTRL(i));
	}
}

static void tlv320aic3101_adc_cfg(struct snd_soc_component *component)
{
	int i;

	for (i = 0; i < NUM_ADC3101; i++) {
		/* Setup left and right ADC PGA input selection */
		snd_soc_component_write(component, ADC_LEFT_PGA_SEL_1(i), pga1_input_sel_l[i]);
		snd_soc_component_write(component, ADC_RIGHT_PGA_SEL_1(i), pga1_input_sel_r[i]);
		snd_soc_component_write(component, ADC_LEFT_PGA_SEL_2(i), pga2_input_sel_l[i]);
		snd_soc_component_write(component, ADC_RIGHT_PGA_SEL_2(i), pga2_input_sel_r[i]);

		/* Setup PGA gain */
		snd_soc_component_write(component, ADC_LEFT_APGA_CTRL(i), mic_pga_l[i]);
		snd_soc_component_write(component, ADC_RIGHT_APGA_CTRL(i), mic_pga_r[i]);

		/* Unmute ADC channels and set fine gain */
		snd_soc_component_write(component, ADC_ADC_FGA(i), 0x00);
	}
}
			     ...
			     struct snd_soc_dai *dai)
{
	...
	dev_dbg(component->dev, "%s: ##- Data length: %x\n", __func__,
		params_format(params));

	/* DOUT disable: bus keeper disabled */
	for (i = 0; i < NUM_ADC3101; i++)
		snd_soc_component_write(component, ADC_DOUT_CTRL(i), 0x10);
	...
	for (i = 0; i < NUM_ADC3101; i++)
		snd_soc_component_update_bits(component, ADC_INTERFACE_CTRL_1(i),
				    AIC31XX_IFACE1_DATALEN_MASK, data);

	/* Apply gains and unmute */
	tlv320aic3101_adc_cfg(component);

	/* Enable NADC of master */
	snd_soc_component_update_bits(component, ADC_ADC_NADC(0), ADC_ENABLE_NADC,
		ADC_ENABLE_NADC);
	...
	aic31xx_dai_apply_tdm_slot(component);

	/* # Turn Douts back on */
	/* DOUT: Primary Output Enable, bus keeper disabled */
	for (i = 0; i < NUM_ADC3101; i++)
		snd_soc_component_write(component, ADC_DOUT_CTRL(i), 0x12);
		...
		unsigned int fmt)
{
	...
	case SND_SOC_DAIFMT_CBS_CFM:
		...
		break;
	default:
		dev_alert(component->dev, "Invalid DAI master/slave interface\n");
		return -EINVAL;
	}
	/* interface format */
	...
	case SND_SOC_DAIFMT_LEFT_J:
		...
		break;
	default:
		dev_alert(component->dev, "Invalid DAI interface format\n");
		return -EINVAL;
	}
	...
	if (iface_reg1 & AIC31XX_BCLK_MASTER) {
		int bclk_out = 0;
		/* Calcualte the BCLK out of the first component */
		...
		dev_info(component->dev, "%s: InFreq %d bclk_out %d\n", __func__, aic31xx->freq_in, bclk_out);
		...
		if (i == ARRAY_SIZE(aic31xx_divs_bclk)) {
			dev_err(component->dev, "No matching bclk\n");
			return -EINVAL;
		}
	}
	...
	for (i = 0; i < NUM_ADC3101; i++) {
		/* Inverted BCLK flag */
		snd_soc_component_write(component, ADC_INTERFACE_CTRL_2(i), iface_reg3);
		if (i == 0) {
			/* Set Format and Master mode */
			snd_soc_component_write(component, ADC_INTERFACE_CTRL_1(0),
				iface_reg1);

			if (iface_reg1 & AIC31XX_BCLK_MASTER) {
				/* Enable BCLK and WCLK out when the component is powered down */
				snd_soc_component_write(component, ADC_INTERFACE_CTRL_2(0),
					iface_reg3|AIC31XX_BCLK_WCLK_ACTIVE);
				/* If the first ADC is BCLK master
				 * power BCLK N divider up. Already programmed in set_pll
				 */
				snd_soc_component_update_bits(component, ADC_BCLK_N_DIV(0),
					AIC31XX_BCLK_N_POWER_MASK,
					AIC31XX_BCLK_N_POWER_MASK);
			}
		} else {
			/* Only 1st component can be master */
			snd_soc_component_write(component, ADC_INTERFACE_CTRL_1(i),
				iface_reg1 & (~(AIC31XX_BCLK_MASTER | AIC31XX_WCLK_MASTER)));
			if (divs) {
				/* CODEC_CLKIN = BCLK */
				snd_soc_component_write(component, ADC_CLKGEN_MUX(i),
					AIC31XX_CODEC_CLKIN_BCLK);
				/*  BCLK Divider */
				snd_soc_component_write(component, ADC_BCLK_N_DIV(i), divs->bdiv_n);
				/* P and R divider value */
				snd_soc_component_write(component, ADC_PLL_PROG_PR(i),
						(divs->pll_p & 0x7) << 4 | divs->pll_r);
				/* AOSR */
				snd_soc_component_write(component, ADC_ADC_AOSR(i), divs->aosr & 0xff);
				/* NADC divider value */
				snd_soc_component_write(component, ADC_ADC_NADC(i),
						0x80 | (divs->nadc & 0x7f));
				/* MADC divider value */
				snd_soc_component_write(component, ADC_ADC_MADC(i),
						0x80 | (divs->madc & 0x7f));
			}
		}

		/* Power-up Left ADC and Right ADC */
		snd_soc_component_write(component, ADC_ADC_DIGITAL(i), 0XC2);
	}
		...
		unsigned int freq_out)
{
	...
	case AIC3101_PLL_ADC_FS_CLKIN_BCLK:
		...
		break;
	default:
		dev_err(component->dev, "source %d not supported\n", source);
		return -EINVAL;
	}

	dev_dbg(component->dev, "In aic31xx: dai_set_pll mux=0x%x out=%d in=%d\n", clk_mux, freq_in, freq_out);
	...
	tlv320aic3101_save_settings(component);

	/* Reset ADCs */
	tlv320aic3101_sw_reset(component);

	/*turn off pll and m/n dividers on adc3101*/
	for (i = 0; i < NUM_ADC3101; i++) {
		snd_soc_component_write(component, ADC_PLL_PROG_PR(i), 0x00);
		snd_soc_component_write(component, ADC_ADC_NADC(i), 0x00);
		snd_soc_component_write(component, ADC_ADC_MADC(i), 0x00);
		snd_soc_component_write(component, ADC_BCLK_N_DIV(i), 0X00);
	}

	snd_soc_component_write(component, ADC_CLKGEN_MUX(0), clk_mux);

	snd_soc_component_write(component, ADC_ADC_AOSR(0), divs->aosr & 0xff);

	snd_soc_component_write(component, ADC_PLL_PROG_PR(0),
		pll_enab | (divs->pll_p & 0x7) << ADC_PLLP_SHIFT | divs->pll_r);
	snd_soc_component_write(component, ADC_PLL_PROG_J(0), divs->pll_j & ADC_PLLJ_MASK);
	snd_soc_component_write(component, ADC_PLL_PROG_D_MSB(0),
		(divs->pll_d >> 8) & ADC_PLLD_MSB_MASK);
	snd_soc_component_write(component, ADC_PLL_PROG_D_LSB(0),
		divs->pll_d & ADC_PLLD_LSB_MASK);

	/* NADC divider value. Do not turn on */
	snd_soc_component_write(component, ADC_ADC_NADC(0), (divs->nadc & ADC_NADC_MASK));

	/* MADC divider value */
	snd_soc_component_write(component, ADC_ADC_MADC(0),
	...
	if (pll_id == AIC3101_PLL_BCLK) {
		 ...
		 */
		snd_soc_component_write(component, ADC_BCLK_N_DIV(0), divs->bdiv_n);
	} else if (pll_id == AIC3101_PLL_CLKOUT) {
		/* Add M_Val */
	}

	/* Program DSP */
	aic31xx_apply_tuning(component);

	dev_info(component->dev, "%s: DAI ID %d PLL_ID %d InFreq %d OutFreq %d\n",
 ...
 */
static int aic31xx_set_sysclk(struct snd_soc_component *component,
		int clk_id, int source, unsigned int freq, int dir)
{
	...
	case AIC31XX_FREQ_38400000:
		...
		return 0;
	default:
		dev_err(component->dev, "Invalid frequency to set DAI system clock\n");
		return -EINVAL;
	}
}
				  ...
				  enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_ON:
		for (i = 0; i < NUM_ADC3101; i++)
			snd_soc_component_write(component, ADC_MICBIAS_CTRL(i),
				AIC3X_MICBIAS_2_5V << 5 | AIC3X_MICBIAS_2_5V << 3);
		break;

	case SND_SOC_BIAS_OFF:
		for (i = 0; i < NUM_ADC3101; i++)
			snd_soc_component_write(component, ADC_MICBIAS_CTRL(i), 0);
		break;
	...
	case SND_SOC_BIAS_STANDBY:
		break;
	default:
		dev_err(component->dev, "%s: Invalid bias level=%d\n", __func__, level);
		ret =  -EINVAL;
	}

	snd_soc_component_get_dapm(component)->bias_level = level;

	return ret;
}


static int aic31xx_suspend(struct snd_soc_component *component)
{
	dev_dbg(component->dev, "%s: Entered\n", __func__);

	aic31xx_set_bias_level(component, SND_SOC_BIAS_OFF);
	dev_dbg(component->dev, "%s: Exiting\n", __func__);
	return 0;
}

static int aic31xx_resume(struct snd_soc_component *component)
{
	dev_dbg(component->dev, "%s: Entered\n", __func__);
 ...
 *----------------------------------------------------------------------------
 */
static int tlv320aic3101_init(struct snd_soc_component *component)
{
	struct aic31xx_priv *aic31xx = snd_soc_component_get_drvdata(component);
	int ret;
	int i;
	...
	aic31xx_set_bias_level(component, SND_SOC_BIAS_OFF);
	snd_soc_component_get_dapm(component)->idle_bias_off = true;
...
/*
 * Codec probe function, called upon component registration
 *
 */

static int aic31xx_component_probe(struct snd_soc_component *component)
{
	return tlv320aic3101_init(component);
}

/*
 * Remove aic31xx soc device
 */
static void aic31xx_component_remove(struct snd_soc_component *component)
{
	struct aic31xx_priv *aic31xx = snd_soc_component_get_drvdata(component);
...
static const struct snd_soc_component_driver soc_component_driver_aic31xx = {
	.probe			= aic31xx_component_probe,
	.remove			= aic31xx_component_remove,
...
static int aic31xx_i2c_probe(struct i2c_client *pdev)
{
	...
	if (i == 0) {
		...
		memcpy(aic31xx->reg_cache, aic31xx_reg,
		       sizeof(aic31xx->reg_cache));
		...
		/*
		 * Delay component publication until all ADC3101 I2C clients are
		 * present; component probe initializes every converter.
		 */
	} else {
		aic31xx_global->adc_control_data[i] = pdev;
		i2c_set_clientdata(pdev, aic31xx_global);
	}

	i++;

	if (i == NUM_ADC3101) {
		struct i2c_client *primary = aic31xx_global->adc_control_data[0];

		ret = snd_soc_register_component(&primary->dev,
						&soc_component_driver_aic31xx,
						tlv320aic3101_dai_driver,
						ARRAY_SIZE(tlv320aic3101_dai_driver));
		if (ret) {
			dev_err(&primary->dev,
				"component: %s : registration failed\n",
				__func__);
			i--;
			return ret;
		}
...
{
	struct aic31xx_priv *aic31xx = i2c_get_clientdata(pdev);

	if (aic31xx && aic31xx->adc_control_data[0] == pdev)
...
MODULE_DESCRIPTION("ASoC TLV320AIC3101 component driver");
MODULE_LICENSE("GPL");
